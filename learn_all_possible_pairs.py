...
# Load data
print("Loading data")
(X_train, Y_train) = functions.read_data_with_hog("data/DataSet_Nao_RAW/DataSet_SEQUENCE_1")
...
for i in range(38, 70):
    ...
    (X_train_pairs, Y_train_pairs) = functions.generate_unique_pairs(X_train, Y_train)
    ...
    model_1.load_weights(f'result_unique_pairs/modeltestowy_{i}/model')
    # train
    ...
    os.mkdir(f'result_unique_pairs/model{model_id}')
    os.mkdir(f'result_unique_pairs/model{model_id}/plots')
    model_1.save_weights(f'result_unique_pairs/model{model_id}/model')
    functions.plot_learning_history(his, model_id, hold='_unique_pairs')
